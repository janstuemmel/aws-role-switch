...
const getItemsMock = mock(getStorageItems);
const setItemsMock = mock(setStorageItems);

beforeEach(() => {
  getItemsMock.mockClear();
  setItemsMock.mockClear();
});

it('get undefined config file', async () => {
  mock(getItemsMock).mockResolvedValue({});
...
it('set config file', async () => {
  ...
  `);
  expect(setItemsMock.mock.calls[0][0]).toMatchInlineSnapshot(`
{
