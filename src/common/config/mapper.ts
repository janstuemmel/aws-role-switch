...
const ROLE_ARN_REGEX =
  /^arn:(aws|aws-us-gov):iam::(?<aws_account_id>\d{12}):role\/(?<role_name>[\w+=,.@\\/-]+)/;

const isValidConfigItem = ({
  aws_account_id,
  role_name,
  target_role_name,
}: Partial<AWSConfigItem>): boolean =>
...
export const sortByGroupIndex = (config: AWSConfig) => {
  const groupsIndex = config
    .filter(
      (item: AWSConfigItem, idx: number) =>
        config.findIndex((step) => item.group === step.group) === idx,
    )
...
export const getTargetRoleName = (
  config: StoredConfig,
  source = '',
): string | undefined => {
...
export const getSourceAccountId = (
  config: StoredConfig,
  source = '',
  carry?: string,
): string | undefined => {
...
const buildConfigItem =
  (config: StoredConfig) =>
  (key: string): Partial<AWSConfigItem> => {
    const {
      role_arn = '',
      aws_account_id,
      role_name,
    ...
    const region = availableRegions.includes(regionTo || '')
      ? regionTo
      : undefined;
    ...
    return removeUndefinedEntries({
      ...
      role_name:
        match?.groups?.role_name ??
        role_name ??
        getTargetRoleName(config, source_profile),
